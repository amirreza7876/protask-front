	...
	if (isAuthenticated) {
		if (Object.keys(roomRequests).length !== 0) {
			if (roomRequests.status === 200) {
				return (<div className={'col-span-4 mx-4'}>
					<div className={'grid grid-cols-4 gap-4 place-items-center'}>
						{Object.keys(roomRequests.data).length !== 0 ? roomRequests.data.map(request => (<div
							...
							<div className="flex flex-col items-center">
								...
								{request.status !== 'p' ?
									...
									<p className={`${request.status === 'a' && 'text-green-700'}
